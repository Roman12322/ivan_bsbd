...
namespace kostya
{
    public partial class sim_form : Form
    {
        readonly user _user;
        private SqlConnection sql_connection = null;
        public sim_form(user user)
        {
            _user = user;
            InitializeComponent();
        }

        private void sim_form_Load(object sender, EventArgs e)
        {
            // TODO: данная строка кода позволяет загрузить данные в таблицу "database1DataSet.sim". При необходимости она может быть перемещена или удалена.
            this.simTableAdapter.FillBycustid(this.database1DataSet.sim, _user.id);
